...
static void
variant_unref_checked (gpointer variant)
{
  if (variant != NULL)
  ...
  for (idx = 0; idx < config->download_order->len; ++idx)
    switch (g_array_index (config->download_order,
      ...
      case EOS_UPDATER_DOWNLOAD_MAIN:
        ...
        add_source_variant (source_variants, NULL);
        break;
      ...
      case EOS_UPDATER_DOWNLOAD_VOLUME:
        ...
        add_source_variant (source_variants, g_variant_new_string (config->volume_path));
        break;
